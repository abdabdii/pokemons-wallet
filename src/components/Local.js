...
export const Local = () => {
  ...
  const {pokemons, setPokemons , preRenderPokemons, setPreRenderPokemons ,query , setRawData,rawData , favorites} = useContext(pokemonsContext)
  ...
  useEffect( ()=> {
    ...
    waitPokemons.pokemons = waitPokemons.pokemons.filter((pokemon)=>favorites.includes(pokemon.name))
    ...
    useEffect(() => {
        if(rawData.pokemons){
            setRawData((prev)=>{
                ...
                let updatedPokemons =prev.pokemons.filter((pokemon)=>favorites.includes(pokemon.name))
                
                return{...prev , pokemons:updatedPokemons}})
            setPokemons((prev)=>prev.filter((pokemon)=>favorites.includes(pokemon.name)))
        }
        // eslint-disable-next-line
    }, [favorites])
